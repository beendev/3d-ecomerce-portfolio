...
import SceneMobile from './components/SceneMobile'; // On importe la version mobile
...
export default function Home() {
  ...
  // 1. Détection de la taille d'écran au chargement et au redimensionnement
  useEffect(() => {
    const checkMobile = () => setIsMobile(window.innerWidth < 768);
    checkMobile(); // Vérification immédiate
    window.addEventListener('resize', checkMobile);
    return () => window.removeEventListener('resize', checkMobile);
  }, []);

  // 2. Gestion du Scroll (Step 0 -> 1 -> 2 -> 3)
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      const now = Date.now();
      if (now - lastScrollTime.current < 800) return; // Anti-spam scroll
  ...
  return (
    <main className="relative w-full h-screen bg-[#EBEAE5] text-[#1a1a1a] overflow-hidden font-sans selection:bg-black selection:text-white">
      ...
      {/* --- 3D BACKGROUND INTELLIGENT --- */}
      <div className="absolute inset-0 z-0">
         {/* Si on est sur mobile, on charge SceneMobile, sinon Scene */}
         {isMobile ? (
      ...
      <div className="relative z-10 h-full w-full flex flex-col justify-between p-6 md:p-12 pointer-events-none">
        ...
        <div className={`flex-1 relative w-full flex flex-col justify-center transition-all duration-1000 ${step === 3 ? 'opacity-0 -translate-y-[100vh]' : 'opacity-100 translate-y-0'}`}>
          ...
          {/* TEXTE DÉPLACÉ EN BAS SUR MOBILE POUR NE PAS CACHER LA LUNETTE */}
          <div className="absolute left-0 bottom-12 md:bottom-24 w-full md:max-w-sm pointer-events-auto text-center md:text-left">
             {step < 3 && (
               <div key={step} className="animate-in fade-in slide-in-from-left duration-700">
                  <h2 className="text-4xl md:text-7xl font-serif leading-[0.8] mb-4 md:mb-6 uppercase italic">
        ...
        <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 ease-in-out ${step === 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[100vh]'}`}>
            <div className="text-center w-full max-w-6xl px-4 pointer-events-auto">
                ...
                {/* Le titre avec le trou pour les lunettes */}
